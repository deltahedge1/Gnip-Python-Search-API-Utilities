...
This package includes two utilities: - Gnip Search API interactions
include Search V2 and paging support - Timeseries analysis and plotting

Installation
^^^^^^^^^^^^

Install from PyPI with ``pip install gapi`` Or to use the full time line
capability, ``pip install gapi[timeseries]`` To install locally,
``pip install -e .`` from the top-level directory, or
``pip install -e .[timeseries]``

This installs the package and the 3 utility scripts from the ``tools/``
directory: - ``gnip_search.py`` - ``gnip_time_series.py`` -
...
To avoid entering the the -u, -p and -l options for every command,
...
can omit this command line parameters.

The configuration file can either be named ".gnip" and located in the
directory where you run the code, or you can export the path to the file
as the environment variable "GNIP\_CONFIG\_FILE"
...
Return full, enriched, Activity Streams-format JSON payloads from the
Search API endpoint. Run
Gnip-Python-Search-API-Utilities/gnip\_search.py from
Gnip-Python-Search-API-Utilities:

Note: If you have a GNIP\_CONFIG\_FILE defined (try echo
$GNIP\_CONFIG\_FILE, it should return the path to the config that you
created), -u and -p arguments are not necessary.
...
``-a`` option (paging) collects *all* results before printing to
stdout/file and also forces ``-n 500`` per request. The paging option
...
Return top 1- and 2-grams - with counts and document frequency - from
matching activities. Can modify the settings within simple ngrams
package (``sngrams``) to modify the range of output.
...
Return the most frequently observed links - count and document frequency
...
Return a count timeline of matching activities. Without further options,
...
Return JSON payloads with the latitude, longitude, timestamp, and
activity id for matching activities
...
Return the list of all of the users ids represented by matching
activities
...
.. figure:: /output/img/earthquake_time_line.png
   :alt: Image of Earthquake Timeline

   Image of Earthquake Timeline

.. figure:: /output/img/earthquake_cycle_trend_line.png
   :alt: Image of Earthquake Trend and Variation

   Image of Earthquake Trend and Variation

.. figure:: /output/img/earthquake_time_peaks_line.png
   :alt: Image of Earthquake Peaks

   Image of Earthquake Peaks
...
In addition to the the basic Gnip Search utility described immediately
above, this pakage depends on a number of other large packges:
...
-  You should create the path "output/plots" in the directory where you
   run the utility. This will contain the plots of time series and
   analysis
-  This utility creates an extensive log file named
   log/time\_series.log. It contains many details of parameter settings
   and intermediate outputs.
-  On a remote machine or server, change your matplotlib backend by
   creating a local matplotlibrc file. Create
   Gnip-Python-Search-API-Utilities/matplotlibrc:
...
$ ./tools/gnip\_filter\_analysis.py -h
...
So for this rule set, the redundancy is 89471832/43695763. - 1 =
...
Additionally, csv output of the raw counts and a csv version of the
pivot table are written to the specified data directory.
...
-  Unlike other utilities provided, the defualt file path is set to
   ...
   run the utility. This will contain the data ouputs.

License
-------

Gnip-Python-Search-API-Utilities by Scott Hendrickson, Josh Montague,
